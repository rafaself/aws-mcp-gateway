...
describe("authenticateLocalBearerRequest", () => {
  ...
  it("returns a fresh Response each call so both bodies can be read", async () => {
    ...
    const result1 = authenticateLocalBearerRequest(request1, env);
    const result2 = authenticateLocalBearerRequest(request2, env);
